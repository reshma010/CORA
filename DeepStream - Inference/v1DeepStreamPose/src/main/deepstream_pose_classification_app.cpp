...
/* Helper function to capture and scale frame data for thumbnails */
static bool capture_frame_thumbnail(GstBuffer *gst_buffer, NvDsFrameMeta *frame_meta, SharedMemoryManager *shm_mgr) {
    if (!shm_mgr || !shm_mgr->initialized || !frame_meta || !gst_buffer) {
        return false;
    }

    // Map GstBuffer to get access to NvBufSurface (based on NVIDIA documentation)
    GstMapInfo in_map_info;
    NvBufSurface *surface = NULL;
    memset(&in_map_info, 0, sizeof(in_map_info));
    
    if (!gst_buffer_map(gst_buffer, &in_map_info, GST_MAP_READ)) {
        g_print("Error: Failed to map gst buffer for thumbnail capture\n");
        return false;
    }
    ...
    try {
        ...
        // Calculate scale factor to fit within thumbnail dimensions
        uint32_t orig_width = surf_params->width;
        uint32_t orig_height = surf_params->height;
        
        float scale_w = (float)THUMBNAIL_MAX_WIDTH / orig_width;
        float scale_h = (float)THUMBNAIL_MAX_HEIGHT / orig_height;
        float scale = (scale_w < scale_h) ? scale_w : scale_h;
        
        uint32_t thumb_width = (uint32_t)(orig_width * scale);
        uint32_t thumb_height = (uint32_t)(orig_height * scale);
        
        // Ensure dimensions are reasonable
        if (thumb_width == 0 || thumb_height == 0 || thumb_width > THUMBNAIL_MAX_WIDTH || thumb_height > THUMBNAIL_MAX_HEIGHT) {
            gst_buffer_unmap(gst_buffer, &in_map_info);
            return false;
        }
        
        // Map the surface to CPU for access (for Jetson unified memory)
        if (NvBufSurfaceMap(surface, 0, frame_meta->batch_id, NVBUF_MAP_READ) != 0) {
            gst_buffer_unmap(gst_buffer, &in_map_info);
            return false;
        }
        NvBufSurfaceSyncForCpu(surface, 0, frame_meta->batch_id);
        
        // Create thumbnail data
        uint32_t bytes_per_pixel = 3; // RGB format
        uint32_t thumbnail_size = thumb_width * thumb_height * bytes_per_pixel;
        
        if (thumbnail_size > THUMBNAIL_MAX_SIZE) {
        ...
        shm_mgr->data->has_thumbnail = true;
        shm_mgr->data->thumbnail_width = thumb_width;
        shm_mgr->data->thumbnail_height = thumb_height;
        shm_mgr->data->thumbnail_size = thumbnail_size;
        
        // Use circular buffer for thumbnails
        uint32_t current_index = shm_mgr->data->current_thumbnail_index % THUMBNAIL_BUFFER_COUNT;
        shm_mgr->data->current_thumbnail_index = (current_index + 1) % THUMBNAIL_BUFFER_COUNT;
        
        // Access actual frame data and create thumbnail
        uint8_t *frame_data = (uint8_t *)surf_params->dataPtr;
        uint8_t *thumb_data = shm_mgr->data->thumbnail_buffer[current_index];
        
        if (frame_data) {
            // Simple downsampling - take every nth pixel
            uint32_t x_step = orig_width / thumb_width;
            uint32_t y_step = orig_height / thumb_height;
            
            for (uint32_t y = 0; y < thumb_height; y++) {
                for (uint32_t x = 0; x < thumb_width; x++) {
                    uint32_t src_x = x * x_step;
                    uint32_t src_y = y * y_step;
                    
                    // Calculate source pixel position (assuming RGBA format)
                    uint32_t src_idx = (src_y * orig_width + src_x) * 4;
                    uint32_t dst_idx = (y * thumb_width + x) * 3;
                    
                    // Convert RGBA to RGB
                    if (src_idx + 3 < surf_params->height * surf_params->width * 4) {
                        thumb_data[dst_idx] = frame_data[src_idx];     // R
                        thumb_data[dst_idx + 1] = frame_data[src_idx + 1]; // G
                        thumb_data[dst_idx + 2] = frame_data[src_idx + 2]; // B
                    }
                }
            }
        } else {
            // Fallback: create a gradient pattern if frame data is not accessible
            for (uint32_t i = 0; i < thumbnail_size; i += 3) {
                uint32_t pixel_idx = i / 3;
                uint32_t x = pixel_idx % thumb_width;
                uint32_t y = pixel_idx / thumb_width;
                
                thumb_data[i] = (uint8_t)((x * 255) / thumb_width);     // R
                thumb_data[i+1] = (uint8_t)((y * 255) / thumb_height); // G
                thumb_data[i+2] = 128; // B - constant
            }
        }
...
static void write_detections_to_shared_memory(NvDsBatchMeta *batch_meta, uint32_t frame_num) {
                          ...
                          gpointer u_data)
{
       ...
       l_frame = l_frame->next)
  {
    ...
    // Capture frame thumbnail (only for first frame in batch)
    if (l_frame == batch_meta->frame_meta_list) {
      capture_frame_thumbnail(buf, frame_meta, &g_shm_manager);
    }
  }
  
  // Write detection data to shared memory
  write_detections_to_shared_memory(batch_meta, frame_number);
...
create_source_bin (DsSourceBinStruct *ds_source_struct, gchar * uri)
{
  ...
  if (g_str_has_prefix (uri, "csi://")) {
    ...
    g_print ("Creating CSI camera source: sensor_id=%d, %dx%d@%d fps\n",
    ...
    if (!camera_src || !ds_source_struct->nvvidconv || !ds_source_struct->capsfilt) {
      g_printerr ("One element in CSI source bin could not be created.\n");
      return false;
    }
    ...
    /* Create camera caps for nvarguscamerasrc */
    camera_caps = gst_caps_new_simple ("video/x-raw(memory:NVMM)",
        "width", G_TYPE_INT, csi_params.width,
        "height", G_TYPE_INT, csi_params.height,
        "format", G_TYPE_STRING, "NV12",
        "framerate", GST_TYPE_FRACTION, csi_params.fps_n, csi_params.fps_d,
        NULL);
    
    /* Set output caps for camera */
    GstElement *camera_capsfilter = gst_element_factory_make ("capsfilter", "camera_capset");
    ...
    gst_bin_add_many (GST_BIN (ds_source_struct->source_bin),
        camera_src, camera_capsfilter, ds_source_struct->nvvidconv,
        ds_source_struct->capsfilt, NULL);
    
    /* Link elements: camera_src -> camera_capsfilter -> nvvidconv -> capsfilt */
    if (!gst_element_link_many (camera_src, camera_capsfilter, 
                               ds_source_struct->nvvidconv, ds_source_struct->capsfilt, NULL)) {
      g_printerr ("Could not link CSI camera elements\n");
      return false;
    }
