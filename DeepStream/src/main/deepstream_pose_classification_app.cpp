...
// Shared memory manager for pose detection data
static SharedMemoryManager g_shm_manager;
...
/* Helper function to capture native format thumbnail for a detected object */
static bool capture_object_thumbnail(GstBuffer *gst_buffer, NvDsFrameMeta *frame_meta, NvDsObjectMeta *obj_meta, uint8_t *thumbnail_buffer, uint32_t *thumb_width, uint32_t *thumb_height, uint32_t *thumb_size, uint32_t *color_format) {
    if (!frame_meta || !gst_buffer || !obj_meta || !thumbnail_buffer || !thumb_width || !thumb_height || !thumb_size || !color_format) {
        return false;
    }

    // Map GstBuffer to get access to NvBufSurface
    GstMapInfo in_map_info;
    NvBufSurface *surface = NULL;
    memset(&in_map_info, 0, sizeof(in_map_info));
    
    if (!gst_buffer_map(gst_buffer, &in_map_info, GST_MAP_READ)) {
        return false;
    }
    ...
    // Validate batch_id bounds
    if (frame_meta->batch_id >= surface->numFilled) {
        gst_buffer_unmap(gst_buffer, &in_map_info);
        return false;
    }
    ...
    // Log format only once for info
    static bool format_logged = false;
    if (!format_logged) {
        g_print("INFO: Per-object thumbnail capture using native format %d\n", surf_params->colorFormat);
        format_logged = true;
    }
    
    try {
        // Map surface for CPU access
        if (NvBufSurfaceMap(surface, 0, frame_meta->batch_id, NVBUF_MAP_READ) != 0) {
            gst_buffer_unmap(gst_buffer, &in_map_info);
            return false;
        }
        
        if (NvBufSurfaceSyncForCpu(surface, 0, frame_meta->batch_id) != 0) {
            NvBufSurfaceUnMap(surface, 0, frame_meta->batch_id);
            gst_buffer_unmap(gst_buffer, &in_map_info);
            return false;
        }

        // Get crop region from object bounding box
        uint32_t frame_width = surf_params->width;
        uint32_t frame_height = surf_params->height;
        
        // Calculate crop region (expand bbox slightly for context)
        float bbox_left = obj_meta->rect_params.left;
        float bbox_top = obj_meta->rect_params.top;
        float bbox_width = obj_meta->rect_params.width;
        float bbox_height = obj_meta->rect_params.height;
        
        // Add 10% padding around detection
        float padding = 0.1f;
        float pad_x = bbox_width * padding;
        float pad_y = bbox_height * padding;
        
        uint32_t crop_x = (uint32_t)fmax(0, bbox_left - pad_x);
        uint32_t crop_y = (uint32_t)fmax(0, bbox_top - pad_y);
        uint32_t crop_w = (uint32_t)fmin(frame_width - crop_x, bbox_width + 2*pad_x);
        uint32_t crop_h = (uint32_t)fmin(frame_height - crop_y, bbox_height + 2*pad_y);
        
        // Calculate downscale to fit within thumbnail limits
        float scale_w = (float)THUMBNAIL_MAX_WIDTH / crop_w;
        float scale_h = (float)THUMBNAIL_MAX_HEIGHT / crop_h;
        float scale = fmin(scale_w, scale_h);
        scale = fmin(scale, 1.0f); // Never upscale
        
        *thumb_width = (uint32_t)(crop_w * scale);
        *thumb_height = (uint32_t)(crop_h * scale);
        
        // Store native format info for monitor to use
        *color_format = surf_params->colorFormat;
        
        // For native format storage, we need to store format-specific metadata
        // Store: [format][width][height][crop_x][crop_y][crop_w][crop_h][scale][data...]
        uint32_t *header = (uint32_t*)thumbnail_buffer;
        header[0] = *color_format;
        header[1] = *thumb_width;
        header[2] = *thumb_height; 
        header[3] = crop_x;
        header[4] = crop_y;
        header[5] = crop_w;
        header[6] = crop_h;
        *(float*)&header[7] = scale;
        
        uint32_t header_size = 8 * sizeof(uint32_t);
        uint8_t *data_ptr = thumbnail_buffer + header_size;
        uint32_t max_data_size = THUMBNAIL_MAX_SIZE - header_size;
        
        // Store raw cropped data in native format for monitor to convert
        uint8_t *frame_data = (uint8_t *)surf_params->dataPtr;
        uint32_t pitch = surf_params->planeParams.pitch[0];
        
        // Simple crop and downsample of raw data
        uint32_t bytes_per_pixel = 1; // Default for most YUV formats
        if (surf_params->colorFormat == NVBUF_COLOR_FORMAT_RGBA || 
            surf_params->colorFormat == NVBUF_COLOR_FORMAT_BGRA) {
            bytes_per_pixel = 4;
        } else if (surf_params->colorFormat == NVBUF_COLOR_FORMAT_RGB ||
                   surf_params->colorFormat == NVBUF_COLOR_FORMAT_BGR) {
            bytes_per_pixel = 3;
        }
        
        uint32_t data_size = 0;
        uint32_t step_x = crop_w / *thumb_width;
        uint32_t step_y = crop_h / *thumb_height;
        step_x = (step_x == 0) ? 1 : step_x;
        step_y = (step_y == 0) ? 1 : step_y;
        
        for (uint32_t y = 0; y < *thumb_height && data_size + bytes_per_pixel <= max_data_size; y++) {
            uint32_t src_y = crop_y + (y * step_y);
            if (src_y >= frame_height) break;
            
            for (uint32_t x = 0; x < *thumb_width && data_size + bytes_per_pixel <= max_data_size; x++) {
                uint32_t src_x = crop_x + (x * step_x);
                if (src_x >= frame_width) break;
                
                uint32_t src_idx = src_y * pitch + src_x * bytes_per_pixel;
                if (src_idx + bytes_per_pixel <= surf_params->dataSize) {
                    memcpy(data_ptr + data_size, frame_data + src_idx, bytes_per_pixel);
                    data_size += bytes_per_pixel;
                }
            }
        }
        
        *thumb_size = header_size + data_size;
...
/* Old frame-based thumbnail function removed - now using per-object thumbnails */

// Temporary structure to hold thumbnail data for an object
typedef struct {
    uint64_t object_id;
    uint32_t thumbnail_width;
    uint32_t thumbnail_height; 
    uint32_t thumbnail_size;
    uint8_t thumbnail_data[THUMBNAIL_MAX_SIZE];
    bool has_thumbnail;
} ObjectThumbnail;

/* Helper function to extract detection data and write to shared memory */
static void write_detections_to_shared_memory(GstBuffer *gst_buffer, NvDsBatchMeta *batch_meta, uint32_t frame_num) {
    if (!g_shm_enabled || !g_shm_manager.initialized) {
        return;
    }
    
    PersonDetection detections[MAX_PERSONS];
    ...
    for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL && detection_count < MAX_PERSONS; 
         l_frame = l_frame->next) {
        NvDsFrameMeta *frame_meta = (NvDsFrameMeta *)(l_frame->data);
        
        for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj != NULL && detection_count < MAX_PERSONS; 
             l_obj = l_obj->next) {
            NvDsObjectMeta *obj_meta = (NvDsObjectMeta *)(l_obj->data);
            
            if (obj_meta->class_id == PGIE_CLASS_ID_PERSON) {
                PersonDetection *detection = &detections[detection_count];
                
                // Basic detection info
                detection->person_id = obj_meta->object_id;
                detection->timestamp_us = timestamp;
                detection->frame_number = frame_num;
                
                // Bounding box
                detection->bbox.left = obj_meta->rect_params.left;
                detection->bbox.top = obj_meta->rect_params.top;
                detection->bbox.width = obj_meta->rect_params.width;
                detection->bbox.height = obj_meta->rect_params.height;
                detection->bbox.confidence = obj_meta->confidence;
                
                // Tracking info
                detection->is_tracked = (obj_meta->object_id != UNTRACKED_OBJECT_ID);
                detection->tracking_age = 1; // Could be enhanced with actual tracking age
                
                // Process user metadata for pose data
                for (NvDsMetaList *l_user = obj_meta->obj_user_meta_list; l_user != NULL; 
                     l_user = l_user->next) {
                    NvDsUserMeta *user_meta = (NvDsUserMeta *)l_user->data;
                    
                    // Check for pose joints metadata
                    if (user_meta->base_meta.meta_type == NVDS_OBJ_META) {
                        NvDsJoints *joints_meta = (NvDsJoints *)user_meta->user_meta_data;
                        if (joints_meta && joints_meta->joints && joints_meta->pose_type == 1) {
                            // Copy 3D pose joints
                            for (int i = 0; i < MAX_JOINTS && i < (int)joints_meta->num_joints; i++) {
                                detection->joints_3d[i].x = joints_meta->joints[i].x;
                                detection->joints_3d[i].y = joints_meta->joints[i].y; 
                                detection->joints_3d[i].z = joints_meta->joints[i].z;
                                detection->joints_3d[i].confidence = joints_meta->joints[i].confidence;
                                detection->joints_3d[i].visible = (joints_meta->joints[i].confidence > 0.1f);
                            }
                            detection->has_3d_pose = true;
                        }
                    }
                    
                    // Check for pose classification metadata
                    if (user_meta->base_meta.meta_type == NVDSINFER_TENSOR_OUTPUT_META) {
                        NvDsInferTensorMeta *tensor_meta = (NvDsInferTensorMeta *)user_meta->user_meta_data;
                        // Process classification results if available
                        // This would need to be enhanced based on the specific tensor output format
                    }
                }
                
                // Check for classifier metadata (pose classification)
                for (NvDsMetaList *l_classifier = obj_meta->classifier_meta_list; l_classifier != NULL;
                     l_classifier = l_classifier->next) {
                    NvDsClassifierMeta *classifier_meta = (NvDsClassifierMeta *)l_classifier->data;
                    
                    for (NvDsMetaList *l_label = classifier_meta->label_info_list; l_label != NULL;
                         l_label = l_label->next) {
                        NvDsLabelInfo *label_info = (NvDsLabelInfo *)l_label->data;
                        
                        // Map classification results to pose classes
                        if (label_info->result_class_id < MAX_POSE_CLASSES) {
                            detection->pose_scores[label_info->result_class_id] = label_info->result_prob;
                            
                            // Find the class with highest confidence
                            if (label_info->result_prob > detection->pose_confidence) {
                                detection->pose_confidence = label_info->result_prob;
                                detection->pose_class = (PoseClass)label_info->result_class_id;
                            }
                        }
                    }
                    detection->has_classification = true;
                }
                
                // Capture thumbnail for this detected object in native format
                detection->has_thumbnail = false;
                uint32_t thumb_width = 0, thumb_height = 0, thumb_size = 0, color_format = 0;
                
                if (capture_object_thumbnail(gst_buffer, frame_meta, obj_meta, 
                                           detection->thumbnail_data, 
                                           &thumb_width, &thumb_height, 
                                           &thumb_size, &color_format)) {
                    detection->has_thumbnail = true;
                    detection->thumbnail_width = thumb_width;
                    detection->thumbnail_height = thumb_height;
                    detection->thumbnail_size = thumb_size;
                    
                    g_print("Captured thumbnail for object %lu: %dx%d, %u bytes, format %u\n", 
                            obj_meta->object_id, thumb_width, thumb_height, thumb_size, color_format);
                } else {
                    g_print("Failed to capture thumbnail for object %lu\n", obj_meta->object_id);
                }
    ...
    // Write detection data to shared memory
    if (detection_count > 0 || g_shm_manager.data->pipeline_active) {
        // Update FPS info
        ...
        if (current_time - last_fps_time >= 1000000) { // Update every second
            g_shm_manager.data->fps = fps_frame_count;
            fps_frame_count = 0;
            last_fps_time = current_time;
        }
        
        // Set frame dimensions
        g_shm_manager.data->frame_width = _image_width;
        g_shm_manager.data->frame_height = _image_height;
        
        shm_write_detection_data(&g_shm_manager, detections, detection_count, frame_num, timestamp);
    }
}
                          ...
                          gpointer u_data)
{
       ...
       l_frame = l_frame->next)
  {
    ...
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next)
    {
      ...
      // Note: Thumbnail capture will be handled inside write_detections_to_shared_memory
      // to ensure proper integration with shared memory structure
    }
  ...
  // Write detection data to shared memory (including per-object thumbnails)
  write_detections_to_shared_memory(buf, batch_meta, frame_number);
...
/* Probe function to drop empty batched buffers that can cause crashes */
static GstPadProbeReturn drop_empty_buffer (GstPad * pad,
    ...
    if (batch_meta && batch_meta->num_frames_in_batch == 0) {
      g_print("Dropping empty buffer to prevent crash\n");
...
// ---Oleg Korobeyko hh9612---

/* Structure to hold CSI camera parameters */
typedef struct {
  gint sensor_id;
  gint width;
  gint height;
 ...
 * Expected format: csi://0?width=1920&height=1080&fps=30
 */
static gboolean
parse_csi_uri_params (const gchar * uri, CsiCameraParams * params)
{
  gchar **uri_parts = NULL;
  gchar **query_parts = NULL;
  gboolean ret = FALSE;
  ...
  if (!g_str_has_prefix (uri, "csi://")) {
    return FALSE;
  }
  
  /* Parse sensor_id from csi://0 part */
  gchar *uri_copy = g_strdup (uri);
  gchar *question_mark = g_strstr_len (uri_copy, -1, "?");
  
  if (question_mark) {
    *question_mark = '\0';
    gchar *query_string = question_mark + 1;
    
    /* Parse query parameters */
    query_parts = g_strsplit (query_string, "&", 0);
    for (gint i = 0; query_parts && query_parts[i]; i++) {
      gchar **param_parts = g_strsplit (query_parts[i], "=", 2);
      if (param_parts && param_parts[0] && param_parts[1]) {
        if (g_strcmp0 (param_parts[0], "width") == 0) {
          params->width = g_ascii_strtoll (param_parts[1], NULL, 10);
        } else if (g_strcmp0 (param_parts[0], "height") == 0) {
          params->height = g_ascii_strtoll (param_parts[1], NULL, 10);
        } else if (g_strcmp0 (param_parts[0], "fps") == 0) {
          params->fps_n = g_ascii_strtoll (param_parts[1], NULL, 10);
        }
      }
      g_strfreev (param_parts);
    }
    g_strfreev (query_parts);
  }
  
  /* Parse sensor_id from csi://N part */
  gchar *sensor_id_str = uri_copy + 6; /* Skip "csi://" */
  if (sensor_id_str && *sensor_id_str) {
    params->sensor_id = g_ascii_strtoll (sensor_id_str, NULL, 10);
  }
...
create_source_bin (DsSourceBinStruct *ds_source_struct, gchar * uri)
{
  ...
  /* Check if this is a CSI camera URI */
  if (g_str_has_prefix (uri, "csi://")) {
    /* Handle CSI camera source */
    CsiCameraParams csi_params;
    GstElement *camera_src = NULL;
    GstCaps *camera_caps = NULL;
    
    if (!parse_csi_uri_params (uri, &csi_params)) {
      g_printerr ("Failed to parse CSI URI parameters: %s\n", uri);
      return false;
    }
    
    g_print ("DEBUG: Creating CSI camera source: sensor_id=%d, %dx%d@%d fps\n",
        csi_params.sensor_id, csi_params.width, csi_params.height, csi_params.fps_n);
    
    /* Create camera source elements */
    g_print ("DEBUG: Creating nvarguscamerasrc element...\n");
    camera_src = gst_element_factory_make ("nvarguscamerasrc", "camera_src");
    g_print ("DEBUG: Creating nvvideoconvert element...\n");
    ds_source_struct->nvvidconv = gst_element_factory_make ("nvvideoconvert", "source_nvvidconv");
    g_print ("DEBUG: Creating capsfilter element...\n");
    ds_source_struct->capsfilt = gst_element_factory_make ("capsfilter", "source_capset");
    
    if (!camera_src || !ds_source_struct->nvvidconv || !ds_source_struct->capsfilt) {
      g_printerr ("ERROR: One element in CSI source bin could not be created.\n");
      g_printerr ("  camera_src: %p, nvvidconv: %p, capsfilt: %p\n", 
                  camera_src, ds_source_struct->nvvidconv, ds_source_struct->capsfilt);
      return false;
    }
    g_print ("DEBUG: All CSI elements created successfully\n");
    
    /* Set compute-hw=VIC and proper memory type for CSI camera compatibility on Jetson */
    g_print ("DEBUG: Setting nvvideoconvert properties...\n");
    g_object_set(G_OBJECT(ds_source_struct->nvvidconv), 
                 "compute-hw", 1, /* VIC=1 (recommended for Jetson) */
                 "nvbuf-memory-type", 0, /* nvbuf-mem-surface-array */
                 NULL);
    
    /* Set camera properties */
    g_print ("DEBUG: Setting camera sensor-id=%d...\n", csi_params.sensor_id);
    g_object_set (G_OBJECT (camera_src), 
                  "sensor-id", csi_params.sensor_id, 
                  NULL);
    g_print ("DEBUG: Camera properties set successfully\n");
    
    /* No immediate capsfilter after nvarguscamerasrc to prevent hanging - let it negotiate naturally */
    
    /* Add queue elements for better buffering and sync */
    GstElement *queue1 = gst_element_factory_make ("queue", "queue1");
    GstElement *queue2 = gst_element_factory_make ("queue", "queue2");
    
    if (!queue1 || !queue2) {
      g_printerr ("Could not create queue elements\n");
      return false;
    }
    
    /* Configure queues for CSI camera - prevent buffer overrun */
    ...
    /* Create final output caps */
    g_print ("DEBUG: Creating final output caps...\n");
    ...
    /* Add elements to bin - no camera_capsfilter to prevent hanging */
    g_print ("DEBUG: Adding elements to source bin...\n");
    gst_bin_add_many (GST_BIN (ds_source_struct->source_bin),
        camera_src, queue1, ds_source_struct->nvvidconv,
        queue2, ds_source_struct->capsfilt, NULL);
    
    /* Link elements: camera_src -> queue1 -> nvvidconv -> queue2 -> capsfilt */
    g_print ("DEBUG: Linking CSI camera elements...\n");
    if (!gst_element_link_many (camera_src, queue1,
                               ds_source_struct->nvvidconv, queue2, ds_source_struct->capsfilt, NULL)) {
      g_printerr ("ERROR: Could not link CSI camera elements\n");
      return false;
    }
    g_print ("DEBUG: CSI camera elements linked successfully\n");
    
  } else {
    /* Handle regular URI sources (files, RTSP, etc.) */
  ...
  /* Create ghost pad for the source bin */
...
int main(int argc, char *argv[])
{
  ...
  /* Initialize shared memory for pose detection data */
  if (!shm_init(&g_shm_manager)) {
    g_printerr("Failed to initialize shared memory. Continuing without shared memory support.\n");
    g_shm_enabled = false;
  } else {
    g_print("Shared memory initialized successfully for pose detection data\n");
  }

  /* Create gstreamer elements */
  /* Create Pipeline element that will form a connection of other elements */
  g_print("DEBUG: Creating main pipeline...\n");
  pipeline = gst_pipeline_new("deepstream_pose_classfication_app");
  if (!pipeline) {
    g_printerr ("ERROR: Pipeline could not be created. Exiting.\n");
    return -1;
  }
  g_print("DEBUG: Main pipeline created successfully\n");
  ...
  for (iterator = g_list, num_sources=0; iterator; iterator = iterator->next,num_sources++) {
    ...
    if (g_strrstr ((gchar *)iterator->data, "rtsp://") ||
        ...
        g_strrstr ((gchar *)iterator->data, "csi://")) {
      isStreaming = true;
    } else {
      isStreaming = false;
    }

    g_print("DEBUG: Processing video source %s\n", (gchar *)iterator->data);

    g_print("DEBUG: Creating source bin for source %d...\n", num_sources);
    if (!create_source_bin (&(source_struct[num_sources]), (gchar *)iterator->data))
    {
      g_printerr ("ERROR: Source bin could not be created. Exiting.\n");
      return -1;
    }
    g_print("DEBUG: Source bin %d created successfully\n", num_sources);
  ...
  /* Set compute-hw=GPU for better compatibility */
  g_object_set(G_OBJECT(nvvidconv), "compute-hw", 2, NULL);
  ...
  /* Set compute-hw=GPU for better compatibility */
  g_object_set(G_OBJECT(nvvideoconvert_enlarge), "compute-hw", 2, NULL);
  ...
  /* Set compute-hw=GPU for better compatibility */
  g_object_set(G_OBJECT(nvvideoconvert_reduce), "compute-hw", 2, NULL);
  ...
  const int muxer_output_width_pad = _pad_dim * 2 + _image_width;
  const int muxer_output_height_pad = _pad_dim * 2 + _image_height;
  asprintf (&string1, "video/x-raw,width=%d,height=%d",
      muxer_output_width_pad, muxer_output_height_pad);
  GstCaps *caps1 = gst_caps_from_string (string1);
  GstCapsFeatures *caps1_feature = gst_caps_features_new ("memory:NVMM", NULL);
  gst_caps_set_features (caps1, 0, caps1_feature);
  ...
  GstCapsFeatures *caps1_feature2 = gst_caps_features_new ("memory:NVMM", NULL);
  gst_caps_set_features (caps1, 0, caps1_feature2);
  ...
  /* Add probe to tiler sink pad to drop empty buffers - prevents crashes */
  GstPad *tiler_sinkpad = gst_element_get_static_pad(nvtile, "sink");
  if (tiler_sinkpad) {
    gst_pad_add_probe(tiler_sinkpad, GST_PAD_PROBE_TYPE_BUFFER, drop_empty_buffer,
                      NULL, NULL);
    gst_object_unref(tiler_sinkpad);
  }
  ...
  /* Set the pipeline to "playing" state with detailed debugging */
  g_print("DEBUG: Setting pipeline to PLAYING state...\n");
  
  /* First try PAUSED to see where it hangs */
  g_print("DEBUG: Transitioning to PAUSED state first...\n");
  GstStateChangeReturn ret = gst_element_set_state(pipeline, GST_STATE_PAUSED);
  
  if (ret == GST_STATE_CHANGE_FAILURE) {
    g_printerr ("ERROR: Unable to set the pipeline to PAUSED state.\n");
    gst_object_unref(pipeline);
    return -1;
  }
  
  g_print("DEBUG: Waiting for PAUSED state change...\n");
  GstState state, pending;
  ret = gst_element_get_state(pipeline, &state, &pending, GST_CLOCK_TIME_NONE);
  
  if (ret == GST_STATE_CHANGE_FAILURE) {
    g_printerr ("ERROR: Failed to change pipeline state to PAUSED.\n");
    gst_object_unref(pipeline);
    return -1;
  }
  
  g_print("DEBUG: Pipeline is now in PAUSED state. Transitioning to PLAYING...\n");
  ...
  g_print("DEBUG: Waiting for PLAYING state change...\n");
  ret = gst_element_get_state(pipeline, &state, &pending, GST_CLOCK_TIME_NONE);
  
  if (ret == GST_STATE_CHANGE_FAILURE) {
    g_printerr ("ERROR: Failed to change pipeline state to PLAYING.\n");
    gst_object_unref(pipeline);
    return -1;
  }
  
  g_print("SUCCESS: Pipeline is now PLAYING!\n");
  GST_DEBUG_BIN_TO_DOT_FILE((GstBin*)pipeline, GST_DEBUG_GRAPH_SHOW_ALL, "pipeline");

  /* Wait till pipeline encounters an error or EOS */
  g_print("Running...\n");
  
  /* Watchdog timeout removed - pipeline runs indefinitely */
  ...
  /* Mark pipeline as inactive in shared memory */
  if (g_shm_enabled && g_shm_manager.data) {
    g_shm_manager.data->pipeline_active = false;
    g_print("Pipeline marked as inactive in shared memory\n");
  }

  /* Cleanup shared memory */
  if (g_shm_enabled) {
    shm_cleanup(&g_shm_manager);
    g_print("Shared memory cleaned up\n");
  }
